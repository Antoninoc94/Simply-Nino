...
GrooveStatsURL = function(forceServer)
	...
	if not FILEMAN:DoesFileExist(uat) then
		local boogie
		if forceServer == "boogie" then boogie = true
		elseif forceServer == "official" then boogie = false
		else boogie = ThemePrefs.Get("EnableBoogieStats") end
...
RequestResponseActor = function(x, y, forceServer)
	local url_prefix = GrooveStatsURL(forceServer)
